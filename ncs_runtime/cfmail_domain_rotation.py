...
def make_domain_attempt(
    ...
    """Convenience constructor from registration result fields."""
    domain = email.rsplit("@", 1)[-1].strip().lower() if "@" in email else ""
    if not domain:
        return None
    blacklist_code = ""
    backend_failure = False
    if error_code in BLACKLIST_ERROR_CODES or error_code in MAILBOX_REUSED_ERROR_CODES:
        blacklist_code = error_code
    elif "mailbox" in error_message.lower() or "mail" in error_message.lower():
        backend_failure = True
    return DomainAttempt(
        domain=domain,
        stage="create_account" if not success else "completed",
        success=success,
        proxy_key=proxy_key,
