"""
OpenAI 协议注册机 (Protocol Keygen) v6 — curl_cffi TLS 指纹增强版
================================================================
协议注册机实现（合并 curl_cffi 随机 Chrome 指纹 + Cloudflare Worker 邮箱）
...
def load_config():
    """加载配置：优先 config.json，fallback 环境变量"""
...
# 基础配置
TOTAL_ACCOUNTS = int(_config.get("total_accounts", 0)) or 30
CONCURRENT_WORKERS = int(_config.get("concurrent_workers", 0)) or 1
HEADLESS = _config.get("headless", True)
PROXY = _config.get("proxy", "") or os.environ.get("PROXY", "")

# 邮箱配置（优先环境变量）
CF_WORKER_DOMAIN = os.environ.get("CFMAIL_WORKER_DOMAIN", "") or _config.get("cf_worker_domain", "")
CF_EMAIL_DOMAIN = os.environ.get("CFMAIL_EMAIL_DOMAIN", "") or _config.get("cf_email_domain", "")
CF_ADMIN_PASSWORD = os.environ.get("CFMAIL_ADMIN_PASSWORD", "") or _config.get("cf_admin_password", "")
...
# 上传配置（优先环境变量）
UPLOAD_API_URL = os.environ.get("UPLOAD_API_URL", "") or _config.get("upload_api_url", "")
UPLOAD_API_TOKEN = os.environ.get("UPLOAD_API_TOKEN", "") or _config.get("upload_api_token", "")
...
# =================== Chrome 指纹配置（从 chatgpt_register.py 移植） ===================
...
def _random_chrome_version():
    """随机选择 Chrome 版本，返回 (impersonate, major, full_version, user_agent, sec_ch_ua)"""
    ...
    return profile["impersonate"], major, full_version, ua, profile["sec_ch_ua"]


# =================== HTTP 会话管理 ===================

# 默认 Chrome 指纹（模块级，用于独立的辅助函数）
_DEFAULT_IMPERSONATE, _DEFAULT_CHROME_MAJOR, _DEFAULT_CHROME_FULL, USER_AGENT, _DEFAULT_SEC_CH_UA = _random_chrome_version()


def create_session(impersonate=None):
    """创建带 Chrome TLS 指纹的 HTTP 会话（curl_cffi）"""
    imp = impersonate or _DEFAULT_IMPERSONATE
...
def _build_common_headers(ua=None, sec_ch_ua=None):
    """动态构建 API 请求头（适配随机 Chrome 版本）"""
    return {
...
def _build_navigate_headers(ua=None, sec_ch_ua=None):
    """动态构建页面导航请求头（适配随机 Chrome 版本）"""
    return {
...
class SentinelTokenGenerator:
    ...
    def _get_config(self):
        ...
        script_version = None
        data_build = random.choice(["en-US", "zh-CN", "en"])
        language = "en-US"
        languages = "en-US,en"
        nav_random2 = random.random()
        # 模拟 navigator 属性值（属性名+实际值）
        nav_prop_values = [
...
def wait_for_verification_code(session, email, cf_token, timeout=300):
    """等待验证邮件并提取验证码"""
...
def _load_oauth_browser_tokens(flows=None, proxy=None, timeout_ms=60000):
    normalized_flows = list(flows or [
        "authorize_continue",
        "password_verify",
        "email_otp_validate",
    ...
    try:
        ...
        print(f"  ❌ OAuth 浏览器 sentinel 初始化失败: {exc}")
        return {flow: None for flow in normalized_flows}
...
def _try_workspace_org_selection_for_code(
    ...
    *,
    ...
    if not org_next:
        return None
    return _follow_existing_session_redirects_for_code(
        session_obj,
...
class ProtocolRegistrar:
    ...
    def step5_create_account(self, first_name, last_name, birthdate):
        ...
        注意：OpenAI 更新后此接口也需要 sentinel PoW 验证，
    ...
    def register(self, email, cf_token, password):
        ...
        try:
            ...
            # ===== 步骤3：触发验证码发送 =====
            # 注册流程需要手动触发 OTP 发送（与 OAuth 登录不同，登录时服务端会自动发）
            self.step3_send_otp()
...
def perform_codex_oauth_login_http(email, password, registrar_session=None, cf_token=None):
    ...
    if cf_token:
        ...
        if initial_emails:
            for e_item in initial_emails:
                if isinstance(e_item, dict) and "id" in e_item:
                    old_mail_ids.add(e_item["id"])
    ...
    def _decode_auth_session(session_obj):
        ...
        """
        val = session_obj.cookies.get("oai-client-auth-session")
        if val:
...
def upload_token_json(filename):
    """上传 Token JSON 文件到 CPA 管理平台（使用 CurlMime）"""
    mp = None
    try:
